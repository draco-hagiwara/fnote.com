...
class Tenpomenu extends CI_Model
{
     ...
     * @return   bool
     */
    public function get_menu_siteid($siteid)
    {

    	$set_where["mn_cl_siteid"] = $siteid;
    ...
    public function get_menu_parent1($siteid)
    {
    	...
    	$sql = 'SELECT mn_seq, mn_id01, mn_id02, mn_id03, mn_name, mn_parent, mn_level, mn_dispno FROM `tb_tenpomenu` '
    ...
    public function get_menulist($arr_post, $tmp_per_page, $tmp_offset=0)
    {
    	...
    	{
    		$set_orderby["mn_level"] = $arr_post['orderid'];
    		$set_orderby["mn_parent"] = $arr_post['orderid'];
    		$set_orderby["mn_dispno"] = $arr_post['orderid'];
    	}else {
...
// //     			WHERE mn_status = 0 AND mn_cl_siteid = '" . $set_select["mn_cl_siteid"] . "'";
     ...
     * @return   array()
     */
    public function delete_menu($seq_no)
    {

    	$where = array('mn_seq' => $seq_no);

    	$result = $this->db->delete('tb_tenpomenu', $where);

    	// ログ書き込み
    	$set_data['lg_user_type'] = 3;
    	$set_data['lg_type']      = 'tenpomenu_delete';
    	$set_data['lg_func']      = 'delete_tenpomenu';
    	$set_data['lg_detail']    = 'mn_seq = ' . $setData['mn_seq'];
    	$this->insert_log($set_data);
