  ...
  <div class="row">
    ...
    <div id="patern05" class="collapse navbar-collapse">
      <ul class="nav navbar-nav">
        ...
        <li class="active"><a href="/site/mp/{$tenpo.tp_cl_siteid}"><i class="glyphicon glyphicon-road"></i> 地図・詳細</a></li>
      </ul>
    </div>
    </nav>
  </div>
...
<H3><p class="bg-info">事業者プラットフォーム　：　インタビュー</p></H3>

<H2>{$tenpo.tp_shopname}　（{$tenpo.tp_shopname_sub}）</H2>

<br>
<form class="form-horizontal" name="detailForm" method="post" action="/site/iv/">
  <table class="table table-striped table-hover">
    <tbody>
      {if $interview.iv_title01!=""}<tr>
        <td>
          <H3>【 {$interview.iv_title01} 】</H3>
          <br><br>
          {$interview.iv_body01}
        </td>
      </tr>{/if}
      {if $interview.iv_title02!=""}<tr>
        <td>
          <H3>【 {$interview.iv_title02} 】</H3>
          <br>
          {$interview.iv_body02}
        </td>
      </tr>{/if}
