...
<H3><p class="bg-info">事業者プラットフォーム　：　メニュー</p></H3>

<H2>{$tenpo.tp_shopname}　（{$tenpo.tp_shopname_sub}）</H2>

<br>
<form class="form-horizontal" name="detailForm" method="post" action="/site/menu/">

<ul class="nav nav-tabs">
  {foreach from=$menu01 key=num item=tabs}
    <li role="presentation" {if $menu_no==$tabs.mn_seq}class="active"{/if}><a href="/site/mn/{$tabs.mn_cl_siteid}/{$tabs.mn_seq}">{$tabs.mn_name} ({$tabs.mn_cnt})</a></li>
  {foreachelse}
    メニューはありません。
  {/foreach}
</ul>
  ...
  {foreach from=$menu02 key=num item=menus}
    <p class="bg-success">{$menus.mn_name} ({$menus.mn_cnt})</p><br>

    {foreach from=$menu03[$num] key=no item=menu}
      {if isset($menu03[$num][$no]['mn_name'])}
        <b style="background-color: rgb(206, 198, 206);">{$menu03[$num][$no]['mn_menu']}　：　{$menu03[$num][$no]['mn_price']}</b><br>
        {$menu03[$num][$no]['mn_info']}<br>
      {/if}
    {/foreach}<br>
